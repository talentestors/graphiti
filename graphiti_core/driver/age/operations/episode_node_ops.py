...
class AGEEpisodeNodeOperations(EpisodeNodeOperations):
    async def save(
        self,
        ...
        query = get_episode_node_save_query(GraphProvider.AGE)
        params: dict[str, Any] = {
            'uuid': node.uuid,
            'name': node.name,
    ...
    async def save_bulk(
        ...
        query = get_episode_node_save_bulk_query(GraphProvider.AGE)
        if tx is not None:
