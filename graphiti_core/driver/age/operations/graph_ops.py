...
class AGEGraphMaintenanceOperations(GraphMaintenanceOperations):
    ...
    async def build_indices_and_constraints(
        ...
        # AGE uses standard Cypher CREATE INDEX syntax (same as Neo4j default)
        range_indices = get_range_indices(GraphProvider.AGE)
    ...
    async def delete_all_indexes(
        ...
        # AGE does not support CALL db.indexes() YIELD name DROP INDEX name.
        # Index management is done via PostgreSQL's DROP INDEX command.
        # Since AGE stores graph data in internal tables, we clear all graph
        # content instead of trying to enumerate indexes.
        # Users who need fine-grained index control should do it via SQL directly.
        logger.warning('delete_all_indexes: AGE does not support programmatic index enumeration. '
                       'Use raw SQL DROP INDEX commands if needed.')
