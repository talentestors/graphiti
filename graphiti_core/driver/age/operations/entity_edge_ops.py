...
class AGEEntityEdgeOperations(EntityEdgeOperations):
    async def save(
        self,
        ...
        query = get_entity_edge_save_query(GraphProvider.AGE)
        if tx is not None:
    ...
    async def save_bulk(
        ...
        query = get_entity_edge_save_bulk_query(GraphProvider.AGE)
        if tx is not None:
